...
import AppStoreApps from './models/appStoreApps'
...
export default {
  getTopFreeApps: async (limit) => {
    try {
      ...
      return Array.isArray(response.data.feed.entry) ? response.data.feed.entry.map(app => new AppStoreApps(app)) : [new AppStoreApps(response.data.feed.entry)]
    } catch (error) {
      throw error
    }
  },
